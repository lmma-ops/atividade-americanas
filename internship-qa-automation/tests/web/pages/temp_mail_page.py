...
class TempMailPage(BasePage):
    URL = "https://temp-mail.io/en"  # sua barra mostra /en

    EMAIL_INPUT = (By.CSS_SELECTOR, "main input[type='text']")  # valor já é o e-mail
    REFRESH_BUTTON = (By.XPATH, "//button[contains(.,'refresh') or contains(.,'Refresh')]")

    # item da lista que contém o assunto "Seu código de acesso"
    MAIL_ROW_SELECTOR = (By.XPATH, "//*[contains(.,'Seu código de acesso')][self::div or self::li or self::a]")

    # região onde fica o texto do e-mail aberto
    MAIL_BODY_CAND = [
    ...
    def wait_first_mail_and_open(self, timeout=45) -> None:
        ...
        while time.time() < end:
            try:
                ...
                try:
                    self.try_click(self.REFRESH_BUTTON)
                except Exception:
                    pass
                time.sleep(2)
    ...
    def extract_code_6d(self) -> str:
        ...
        for loc in self.MAIL_BODY_CAND:
            try:
                ...
                continue
        code = self.find_code_6d(body_text)
        if not code:
            raise AssertionError("Não consegui extrair um código de 6 dígitos do corpo do e-mail.")
        return code
